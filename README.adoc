...
If you already run _movie-api_ in the previous quickstart guides just execute:
...
cd tag-metrics-quickstart-guide/scripts
```
...
This script will load the pre-configured data needed to run this guide.

Now we will check some scenarios to help you better understand how metrics can help you.

== You are a TAG Administrator and an user reports having issues logging in. How do you find the issue?

Let's simulate an user trying to authenticate into TAG and being denied. We will use username bob1.
...
As a TAG Administrator I can use Kibana Discover and the Built in Dashboards to advise the user.

First step is to go to the Discover menu and try to find the request for the username _bob1_. Type the lucene query to filter the request for that user.
```
category:request AND authentication.profiles.username:bob1
```
The request is there, click _JSON_. In the payload authentication property you will see the error code _user_not_found_ saying the user was not found.
...
You can also analyze this request in the OAuth2 Dashboard, therefore copy the request id from the payload and go to the menu _Dashboard_ -> _OAuth2 Dashboard_.

Add the _<request_id_> to the input and press enter. If you check the _Request by Error_ visualization you will see the _user_not_found_ error is there.
...
User will be able to authenticate properly. You can go again to the OAuth2 Dashboard remove the request_id filter and check that the user was able to authenticate.
...
The __Request by Error_ will tell you the reason a request failed, just like we saw previously.
...
== You are a TAG Administrator and you want to advise a _business partner_ that the key is expiring.
...
After that, as TAG Administrator, go to Kibana _Dashboard -> Proxy Dashboard_
...
After executing it, let's go to the _Proxy Dashboard_ again. In the Route visualization you will see that the Route _Movies Microservice 2_, is returning 404, therefore TAG is returning 404 as well. You can see very clear here that TAG found the route to Movies Microservice2 but the back-end didn't have the service.
