...
The docker containers need to be in the same network to be able to communicate. To do that create a specific network for them using:
...
```
docker run --net metrics-net -p 9200:9200 -p 9300:9300 --name=elk_tag -it elasticsearch:5.5
```
...
```
docker run --net metrics-net --name kibana_tag -e ELASTICSEARCH_URL=http://elk_tag:9200 -p 5601:5601 -it kibana:5.5
```
...
```
docker run --net metrics-net --name=tag_metrics -p 8080:8080 -it -e LICENSE=accept -e ELASTICSEARCH_HOST=elk_tag -e KIBANA_HOST=kibana_tag tomitribe/tribestream-api-gateway
```
...
```
docker run --net=metrics-net -d -p 9090:9090 --name movie-api  tomitribedev/movie-api
```
